...
- Keep your `components` folder as a reusable component library, which doen't fetch data from the data layer.
...
- `authors/[user]/page.ts` would only import from `authors/[user]/page.ts`, and not from `posts/data.ts` or `authors/data.ts`.
